...
에이전트(AI)가 TDD 사이클을 자동으로 끝까지 밀어붙이면, 사람이 검토할 틈 없이 잘못된 방향으로 여러 사이클이 누적될 수 있다. **Agentic Engineering**은 에이전트의 속도를 활용하되, 되돌리기 어려운 결정 지점마다 사람의 승인을 받는 것을 의미한다.

### 체크포인트

TDD 사이클에는 다음 4개의 필수 체크포인트가 있다. 각 체크포인트에서 **에이전트는 다음 단계로 자동 진행하지 않고 멈춰서 사람의 확인/승인을 기다린다.**

```
1. PLAN 승인    : 요구사항 분석 + 구현 계획(Plan.md)을 작성 → 사람이 검토 → 승인
2. RED 승인     : Plan을 테스트 코드로 변환 → 사람이 "Plan대로 만들었는지" 확인 → 승인
3. GREEN 승인   : 테스트를 통과시키는 최소 코드 작성 → 사람이 "과하게 구현하지 않았는지" 확인 → 승인
4. REFACTOR 승인: 정리 작업 → 사람이 동작 변경이 없는지 확인 → 승인
```

각 체크포인트를 건너뛰고 다음 단계로 넘어가는 것은 **철의 법칙 위반**과 동급으로 취급한다. 사람이 "진행해"라고 명시적으로 말하기 전까지는 다음 단계의 코드/파일을 작성하지 않는다.

### 왜 필요한가

- 에이전트는 테스트가 실패하는 "이유"는 검증하지만, 그 테스트가 **올바른 요구사항**을 담고 있는지는 사람만 판단할 수 있다.
- Plan.md 단계에서 사람이 방향을 바로잡으면, 이후 RED/GREEN 사이클 전체를 다시 하는 비용을 막을 수 있다.
- "빠르게 여러 사이클을 자동 완료"하는 것은 Agentic Engineering이 아니라 사람의 감독을 우회하는 것이다.

### Plan.md 작성 규칙

- 요구사항에서 도출한 **테스트 케이스 목록**을 우선순위(가장 단순한 것부터) 순서로 나열한다.
- 각 테스트 케이스마다: 무엇을 검증하는지, 왜 그 순서인지(이전 테스트로는 커버되지 않는 새로운 동작인지) 명시한다.
- 구현 세부사항(자료구조, 알고리즘)은 강제하지 않는다 — Plan은 "무엇을 테스트할 것인가"에 집중하고, "어떻게 구현할 것인가"는 GREEN 단계에서 최소한으로 결정한다.

### 사람 검토 시 확인할 것

| 체크포인트 | 사람이 확인할 질문 |
|---|---|
| PLAN | 테스트 목록이 요구사항을 빠짐없이 커버하는가? 불필요한 케이스는 없는가? |
| RED | 테스트 코드가 Plan에 적힌 테스트 케이스와 정확히 일치하는가? 테스트가 올바른 이유로 실패하는가? |
| GREEN | 최소 구현인가? Plan/테스트가 요구하지 않은 기능이 섞여 들어가지 않았는가? |
| REFACTOR | 동작 변경 없이 정리만 되었는가? 여전히 그린인가? |

### 커밋 규칙 (감사 추적)

Human-in-the-Loop가 실제로 지켜졌는지 나중에도 확인할 수 있도록, 각 체크포인트가 끝날 때마다 커밋을 남긴다. 절대 여러 단계를 하나의 커밋으로 합치지 않는다.

```
RED 커밋    : 그 사이클의 테스트 코드 + (있다면) 갱신된 Plan.md 를 커밋
             메시지에 "RED N: ..." 명시
REVIEW 커밋 : GREEN 구현을 사람이 검토("과도한 일반화 없는가")한 뒤 커밋
             메시지에 "GREEN N: ..." 또는 "REVIEW N: ..." 명시
             단, 테스트가 GREEN 단계 없이 즉시 통과했다면(기존 구현이 이미 일반화됨)
             그 사실을 커밋 메시지에 그대로 기록하고 RED 커밋만 남긴다 — 코드 변경이
             없는데 GREEN 커밋을 억지로 만들지 않는다
```

RED 커밋과 REVIEW/GREEN 커밋을 분리하는 이유: 커밋 히스토리만 보고도 "테스트가 먼저 실패했는가 → 그 다음 최소 코드가 추가됐는가"를 검증할 수 있어야 하기 때문이다. 하나로 합치면 이 증거가 사라진다.

### Skill 자기 개선

한 사이클을 마친 뒤, 에이전트가 Agentic Engineering 체크포인트를 실제로 지켰는지 스스로 점검한다. 지키지 않았다면(예: 승인 없이 다음 단계로 진행) 그 사실을 기록하고, 이 Skill 문서에 재발 방지를 위한 규칙을 추가/수정한다.
...
그 외 → TDD가 아니다
계획/테스트/구현 각 단계 전환 → 사람의 명시적 승인이 있었다
```
